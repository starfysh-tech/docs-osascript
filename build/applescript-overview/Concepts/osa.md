<a id="//apple_ref/doc/uid/TP40001571-BABEBGCF"></a>
...
* the ability to create scriptable applications (described in [Scriptable Applications](scriptable_apps.html#//apple_ref/doc/uid/TP40001569-BABEBGCF))
...
* The AppleScript component (in `System/Library/Components`) implements the AppleScript language, which provides a way for scripts to control scriptable applications.
  ...
  The AppleScript language is described in *[AppleScript Language Guide](../../AppleScriptLangGuide/introduction/ASLR_intro.html#//apple_ref/doc/uid/TP40000983)*, as well as in a number of third-party books.
...
Figure 1 shows what happens when the [Script Editor](work_with_as.html#//apple_ref/doc/uid/TP40001568-1152365) application executes an AppleScript script that targets the Mail application. Script Editor calls functions in the Open Scripting framework. The Open Scripting framework communicates through the AppleScript component, which in turn uses the Apple Event Manager to send any required Apple events to the Mail application. If a reply is requested, the Mail application returns information in a reply Apple event.
...
Apple provides a number of scriptable services through agents, as described in [System Events and GUI Scripting](as_related_apps.html#//apple_ref/doc/uid/TP40001570-1149074), [Image Events](as_related_apps.html#//apple_ref/doc/uid/TP40001570-1147921), and [Database Events](as_related_apps.html#//apple_ref/doc/uid/TP40001570-SW1). For example, scripts can use the System Events application to perform operations on property list files.
